...
Изменения внесены в рабочее дерево, **коммит и пуш не выполнялись** — по указанию владельца.
Задачи 9.3 (зелёный workflow) и 9.4 (откат мажоров actions при поломке) остаются открытыми.

**Замечание по триггерам workflow:** он настроен на `push` в `master` и `pull_request`
в `master`. Работа ведётся в ветке `update-to-net10` — пуш этой ветки сам по себе workflow
**не запустит**, нужен pull request в `master` (либо временное расширение списка веток
в триггере). Это стоит учесть при закрытии задачи 9.3.
...
| Зелёный CI-workflow | **не проверено** — коммит и пуш не выполнялись |
...
3. Задачи 9.3–9.4 (зелёный workflow) не закрыты — коммит и пуш остаются за владельцем.
