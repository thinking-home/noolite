...
- [x] 9.3 Отдельным коммитом запушить изменения CI и дождаться зелёного workflow —
      выполнено владельцем: коммит `936440a` «обновлен CI», PR #8, workflow зелёный,
      влито в `master` мерджем `abc5751`
- [x] 9.4 Если workflow падает на мажорах actions — откатить проблемный action на предыдущий
      мажор и зафиксировать причину — **не потребовалось**: `checkout@v7` и `setup-dotnet@v6`
      отработали с первого раза
