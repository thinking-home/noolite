...
> **После архивации режим изменён на `DropWrite`** (решение владельца: порядок прихода честнее,
> см. пометку в design → Decision 3). Счётчик работает и в этом режиме — `itemDropped` вызывается
> для отброшенного нового элемента (проверено по исходникам `BoundedChannel.TryWrite`).
> На живом адаптере с `DropWrite` не перепроверялось: режим `drop` удалён, изменение — одна
> константа; при том же прогоне ожидалось бы `dropped = 8`, но обработались бы первые три
> пакета, а не последние.
