...
обработчикам, **отбрасываются**. `FlushAndCloseAsync()` тоже закрывает порт сразу, но дожидается,
пока обработчики получат всё, что уже лежит в очереди; по возврату очередь пуста. В обоих случаях
`Disconnect` — последнее событие адаптера. `FlushAndCloseAsync()` нельзя вызывать изнутри
обработчика события — он ждал бы сам себя, поэтому бросает `InvalidOperationException`.
